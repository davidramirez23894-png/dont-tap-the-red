...
(() => {
  ...
  function init() {
    ...
    function repositionRed(cfg) {
      ...
      const p = nearPointerPosition(pX, pY, cfg.safeRadius, cfg.huntRadius, rngDaily);
      placeTile(redIndex, p.x, p.y);
    }
    ...
    function applyPhaseTimers() {
      ...
      const tick = () => {
        ...
        const urgencia = 1 - (timeLeftMs / timeLimitMs);
